...
MAX_SECONDS = 7 * 3600
# Сколько ждать распознавание при выходе (шаг 0,5 с). Предел должен быть больше
# худшего случая одного куска: четыре попытки по 180 с плюс паузы — это почти
# 13 минут. Длинный текст к этому моменту уже лежит на диске, так что предел —
# страховка от зависшего потока, а не единственная защита результата.
QUIT_MAX_WAITS = 2400
# Groq принимает файл до 25 МБ. Живой замер на реальном вебинаре: плотная речь
# сжимается в ~1,03 МБ/мин (втрое хуже тихой диктовки), то есть полчаса уже не
# влезает. Режем по 15 минут (~15,5 МБ) — запас держит даже шумную запись, а
# страховкой остаётся проверка фактического размера с делением пополам.
CHUNK_SECONDS = 900
...
class App(tk.Tk):
    ...
    def _work(self, audio):
        ...
        def keep(text):
            ...
            # Пишем из рабочего потока сразу, не дожидаясь возврата в интерфейс:
            # тогда выход из программы в любой момент уже ничего не стоит.
            # Короткую диктовку файлом не захламляем — её потеря не страшна.
            if len(done) > 1 or len(text) > LONG_TEXT_CHARS:
